...
def get_ff_layer(
    ...
    ff_layer = get_attributes(transformer_layers[layer_idx], 'layer')[-1]
    ff_layer = get_attributes(ff_layer, ff_attrs)
    return ff_layer
...
def register_hook(
    ...
    def hook_fn(m, i, o):
        hidden_states = i[0]
        hidden_gelu = m.gelu_act(m.wi_0(hidden_states))
        hidden_linear = m.wi_1(hidden_states)
        activation = hidden_gelu * hidden_linear
        f(activation)
...
class Patch(torch.nn.Module):
    ...
    def forward(self, hidden_states):
        hidden_gelu = self.ff.gelu_act(self.ff.wi_0(hidden_states))
        hidden_linear = self.ff.wi_1(hidden_states)
        hidden_states = hidden_gelu * hidden_linear
        if self.mode == "replace":
            hidden_states[:, self.mask_idx, :] = self.acts
        elif self.mode == "suppress":
            # for pos in self.target_positions:
            hidden_states[:, :, self.target_positions] = 0.0
        elif self.mode == "enhance":
            for pos in self.target_positions:
                hidden_states[:, :, pos] *= self.enhance_value
        else:
            raise NotImplementedError
        hidden_states = self.ff.dropout(hidden_states)
        hidden_states = self.ff.wo(hidden_states)
...
def patch_ff_layer(
    ...
    replacement_activations: torch.Tensor = None,
    mode: str = "replace",
    transformer_layers_attr: str = "decoder.block",
    ff_attrs: str = "DenseReluDense",
    ...
    if mode == "replace":
        ...
        ff_layer_all = get_attributes(transformer_layers[layer_idx], 'layer')[-1]
        ff_layer = get_attributes(ff_layer_all, ff_attrs)
